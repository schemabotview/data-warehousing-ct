...
**Status:** scaffolded + spine agreed (10 modules × ~10 sections — below). `manifest.json` wires the full spine (heading + md/slide/scene paths, `spine:true`, `role:hook` on each §01); `audio` and per-section `focus`/`highlight` get added as sections and scenes are authored. `audio/` is empty by design: the owner generates the `.wav`s from `tts/` via Colab, then the manifest `audio` fields get added back. **Modules 01–05 authored** (`.md` source of truth + distilled `.slide` + `.tts`), wired onto the `dw-architecture` (01) and `star-schema` (02–05) scenes. Modules 03–05 run on the Jabra Spain `FACT_SALES` galaxy model (module 05 covers the schema level — star anatomy, denormalized dims, snowflake + the `DIM_GEOGRAPHY` outrigger, galaxy/fact-constellation, a physical Sales-star walkthrough, PK/SK/FK/NK, degenerate & derived fact columns). `audio/` empty by design (owner generates `.wav`s via Colab); per-section `focus`/`highlight` for modules 02+ get added as the `star-schema` scene node ids settle. Next: module 06 (Slowly Changing Dimensions).
