...
**Status:** scaffolded + spine agreed (10 modules × ~10 sections — below). `manifest.json` wires the full spine (heading + md/slide/scene paths, `spine:true`, `role:hook` on each §01); `audio` and per-section `focus`/`highlight` get added as sections and scenes are authored. `audio/` is empty by design: the owner generates the `.wav`s from `tts/` via Colab, then the manifest `audio` fields get added back. **Modules 01–04 authored** (`.md` source of truth + distilled `.slide` + `.tts`), wired onto the `dw-architecture` (01) and `star-schema` (02–04) scenes. Modules 03–04 run on the Jabra Spain `FACT_SALES` galaxy model (module 04 covers the dimension side — attributes, hierarchies, surrogate keys, conformed/role-playing/junk/degenerate dims, the date dimension). `audio/` empty by design (owner generates `.wav`s via Colab); per-section `focus`/`highlight` for modules 02+ get added as the `star-schema` scene node ids settle. Next: module 05 (Star & Snowflake Schemas).
