...
**Status:** scaffolded + spine agreed (10 modules × ~10 sections — below). Nothing authored yet — `manifest.json` wires the full spine (heading + notebook/slide/scene paths, `spine:true`, `role:hook` on each §01); `audio` and per-section `focus`/`highlight` get added as sections and scenes are authored. `audio/` is empty by design: the owner generates the `.wav`s from `tts/` via Colab, then the manifest `audio` fields get added back. The three scenes (`dw-architecture`, `star-schema`, `datavault-model`) are **not yet built** — they live app-side in `graphl-movie` and get ported next. Next: author module 01 end-to-end.
